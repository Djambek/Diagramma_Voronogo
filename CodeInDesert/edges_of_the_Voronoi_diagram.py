#Данная программа умеет строить диаграмму Вороного с разными параметрами: координаты точек, порядок диаграммы, вес каждого сайта. Программа прорисовывает только рёбра диаграммы Вороного
...
print("введите вес сайтов. Формат ввода: z1 enter z2 enter...")
for i in range(num_of_sites):
...
for i in tqdm(range(1, 200)):
	...
	for j in range(1, 200):
		...
		for p in range(len(ms)):
			massiv.append(float(toFixed((((x1 - ms[p][0])**2 + (y1 - ms[p][1])**2)/weight_of_sites[p]/10), 3)))

		#добавляем координаты ближайших сайтов в mini_massiv
		for c in range(num_of_gardeners+1): 
			mini_massiv.append(min(massiv))      
			massiv.remove(min(massiv))

		if mini_massiv[0] == mini_massiv[1]:
		...
		#(цвет точки зависит от того, какие сайты к ним ближайшие)
			for u in range(len(ms)):
				...
				if (float(toFixed((((x1 - ms[u][0])**2 + (y1 - ms[u][1])**2)/weight_of_sites[u]/10), 3))) in mini_massiv:
					x.append(x1)
					x.append(x1+0.0001)
					...
					ax.plot(x, y, color = [u/10, u/10, 0.4], linewidth = 1)
