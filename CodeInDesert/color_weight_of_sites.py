#данная программа строит дигарамму Вороного, где сайтами являются точки. Она принимает на вход параметры: порядок диаграммы, кол-во сайтов, координаты сайтов и вес каждого сайта.
...
for i in tqdm(range(1, 200)):
	...
	for j in range(1, 200):
		...
		for p in range(len(ms)):
			massiv.append(((x1 - ms[p][0])**2 + (y1 - ms[p][1])**2)/weight_of_sites[p]/10)
		...
		for u in range(len(ms)):
			...
			if (((x1 - ms[u][0])**2 + (y1 - ms[u][1])**2)/weight_of_sites[u]/10) in mini_massiv:
				x.append(x1)
				x.append(x1+0.0001)
